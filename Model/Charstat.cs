using System;
using System.Collections.Generic;

namespace D2SLib2.Model;

public partial class Charstat
{
    public string Class { get; set; } = null!;

    public long? Str { get; set; }

    public long? Dex { get; set; }

    public long? Int { get; set; }

    public long? Vit { get; set; }

    public long? Stamina { get; set; }

    public long? Hpadd { get; set; }

    public long? ManaRegen { get; set; }

    public long? ToHitFactor { get; set; }

    public long? WalkVelocity { get; set; }

    public long? RunVelocity { get; set; }

    public long? RunDrain { get; set; }

    public string? Comment { get; set; }

    public long? LifePerLevel { get; set; }

    public long? StaminaPerLevel { get; set; }

    public long? ManaPerLevel { get; set; }

    public long? LifePerVitality { get; set; }

    public long? StaminaPerVitality { get; set; }

    public long? ManaPerMagic { get; set; }

    public long? StatPerLevel { get; set; }

    public long? SkillsPerLevel { get; set; }

    public long? LightRadius { get; set; }

    public long? BlockFactor { get; set; }

    public long? MinimumCastingDelay { get; set; }

    public string? StartSkill { get; set; }

    public string? Skill1 { get; set; }

    public string? Skill2 { get; set; }

    public string? Skill3 { get; set; }

    public string? Skill4 { get; set; }

    public string? Skill5 { get; set; }

    public string? Skill6 { get; set; }

    public string? Skill7 { get; set; }

    public string? Skill8 { get; set; }

    public string? Skill9 { get; set; }

    public string? Skill10 { get; set; }

    public string? StrAllSkills { get; set; }

    public string? StrSkillTab1 { get; set; }

    public string? StrSkillTab2 { get; set; }

    public string? StrSkillTab3 { get; set; }

    public string? StrClassOnly { get; set; }

    public long? HealthPotionPercent { get; set; }

    public long? ManaPotionPercent { get; set; }

    public string? BaseWclass { get; set; }

    public string? Item1 { get; set; }

    public string? Item1loc { get; set; }

    public long? Item1count { get; set; }

    public long? Item1quality { get; set; }

    public string? Item2 { get; set; }

    public string? Item2loc { get; set; }

    public long? Item2count { get; set; }

    public long? Item2quality { get; set; }

    public string? Item3 { get; set; }

    public string? Item3loc { get; set; }

    public long? Item3count { get; set; }

    public long? Item3quality { get; set; }

    public string? Item4 { get; set; }

    public string? Item4loc { get; set; }

    public long? Item4count { get; set; }

    public long? Item4quality { get; set; }

    public string? Item5 { get; set; }

    public string? Item5loc { get; set; }

    public long? Item5count { get; set; }

    public long? Item5quality { get; set; }

    public string? Item6 { get; set; }

    public string? Item6loc { get; set; }

    public long? Item6count { get; set; }

    public long? Item6quality { get; set; }

    public string? Item7 { get; set; }

    public string? Item7loc { get; set; }

    public long? Item7count { get; set; }

    public long? Item7quality { get; set; }

    public string? Item8 { get; set; }

    public string? Item8loc { get; set; }

    public long? Item8count { get; set; }

    public long? Item8quality { get; set; }

    public string? Item9 { get; set; }

    public string? Item9loc { get; set; }

    public long? Item9count { get; set; }

    public long? Item9quality { get; set; }

    public string? Item10 { get; set; }

    public string? Item10loc { get; set; }

    public long? Item10count { get; set; }

    public long? Item10quality { get; set; }
}
